...
Requires the workspace Google Ads developer token and manager configuration for reporting. Hosted Bob creates a conversation-scoped runtime Google Ads configuration from the connected user's Google OAuth authorization and uses it for approved writes; `google-ads-api.yaml` is not required. A write requires both Google Ads account authorization and Bob account permission `READ & WRITE`. `READ` users can inspect data and prepare plans but cannot apply changes.
