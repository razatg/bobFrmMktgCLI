...
Creates the quarterly static banner design guide from best-performing image assets. Split output:
- `banner-design-strategy.md` — evidence and reasoning
- `DESIGN.md` — the reusable generation spec
- `banner-design.md` — a short landing page linking to both
...
4. Run the static-banner strategist agent/runtime for visual review. In Codex this is configured at `.codex/agents/static-banner-strategist.toml`; other runtimes should use an equivalent visual-analysis agent with:
...
- Thematic generation guidance that maps campaign/ad group/source context into broader creative themes, rather than exact keyword matching.
- Source-reference and prompt-assembly rules for future LOW replacement variants.
- Constraint classes that separate user-confirmed hard SLAs, soft source preservation, design-guide preferences, and inferred brand tokens.
- Do's and don'ts.
