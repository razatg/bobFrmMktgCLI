...
- **Check before fetching.** Before any `./bob fetch`, run `./bob data-manifest --account <active_customer_id> --query <query> --from <from> --to <to>`. Its compact `pulls` section is the authoritative pull history (account, query, date window, outcome), and its `data` section reports local coverage. Do not read or dump `logs/pull-log.jsonl`, grep CSVs, or `ls` raw directories to answer "has this been pulled?". Refetch only if the user says "refetch" or the manifest shows the required window is missing.
