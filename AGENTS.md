...
- Do not say "All set", "You're set up", or show first performance questions until onboarding has verified that the local packages and GARF runner needed for reporting are installed. If the account is saved but local setup is still broken, say that Bob saved the account but is not ready to answer reporting questions yet.
...
- If read access is missing and the user asks a performance/data question, say: "I need the Google Ads developer token from Admin > API Center before I can fetch data from Google Ads." Then stop.
...
./bob resolve-dates --period yesterday-vs-sdlw
./bob resolve-dates --period wow
./bob resolve-dates --period mtd
./bob resolve-dates --period partial-wow          # first 3 days of current week vs prior week (default n=3)
./bob resolve-dates --period partial-wow --n 5   # first 5 days
